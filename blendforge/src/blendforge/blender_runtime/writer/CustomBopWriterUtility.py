...
def write_bop(p, output_dir: str, target_objects: Optional[List[MeshObject]] = None, depths: Optional[List[np.ndarray]] = None, 
    ...
    # Save the data.
    _BopWriterUtility._write_camera(camera_path, depth_scale=depth_scale)
    _BopWriterUtility._write_frames(p, chunks_dir, dataset_objects=dataset_objects, depths=depths, colors=colors,
...
class _BopWriterUtility:
    ...
    def _write_camera(camera_path: str, depth_scale: float = 1.0):
        ...
        _BopWriterUtility._save_json(camera_path, camera)
    ...
    def _get_frame_gt(dataset_objects: List[bpy.types.Mesh], unit_scaling: float, ignore_dist_thres: float,
        ...
        for obj in dataset_objects:
            ...
            H_m2w = Matrix(_WriterUtility.get_common_attribute(obj, 'matrix_world'))
    ...
    @staticmethod
    def _write_frames(p, chunks_dir: str, dataset_objects: list, depths: List[np.ndarray] = [],
        ...
        name_chunk = os.listdir(chunks_dir)
        #print(f'name_chunk {name_chunk}')
        array_chunk = [int(j) for i, j in enumerate(name_chunk)]
        array_chunk = sorted(array_chunk)
        #print(f'array_chunk {array_chunk}')
        last_mb = []
        for i in array_chunk:
            if i in p: 
                last_mb.append(i)
        #print(f'last_mb {last_mb}')
        if len(last_mb):
        
            last_chunk = (last_mb)[-1]
            last_chunk_dir = os.path.join(chunks_dir, f'{last_chunk:06d}')
            last_chunk_gt_fpath = os.path.join(last_chunk_dir, 'scene_gt.json')
            chunk_gt = _BopWriterUtility._load_json(last_chunk_gt_fpath, keys_to_int=True)
        ...
        if curr_frame_id != 0:
            ...
            chunk_gt = _BopWriterUtility._load_json(
                chunk_gt_tpath.format(chunk_id=curr_chunk_id), keys_to_int=True)
            chunk_camera = _BopWriterUtility._load_json(
        ...
        for frame_id in range(bpy.context.scene.frame_start, bpy.context.scene.frame_end):
            ...
            chunk_gt[curr_frame_id] = _BopWriterUtility._get_frame_gt(dataset_objects, unit_scaling, ignore_dist_thres)
            chunk_camera[curr_frame_id] = _BopWriterUtility._get_frame_camera(save_world2cam, depth_scale, unit_scaling)
            ...
            _BopWriterUtility._save_depth(depth_fpath, depth_mm_scaled)
            ...
            if ((curr_frame_id + 1) % frames_per_chunk == 0) or \
                ...
                # Save GT annotations.
                _BopWriterUtility._save_json(chunk_gt_tpath.format(chunk_id=curr_chunk_id), chunk_gt)

                # Save camera info.
                _BopWriterUtility._save_json(chunk_camera_tpath.format(chunk_id=curr_chunk_id), chunk_camera)
