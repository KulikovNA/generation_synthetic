...
from blendforge.blender_runtime.utils import sample_pose_func, sample_pose_func_drop
...
from blendforge.blender_runtime.writer.CustomBopStereoWriterUtility import write_bop_multidepth


def parse_args(argv):
    parser = argparse.ArgumentParser(description="BOP scenario with GT/effective/random depth outputs.")
...
def _prepare_objects_for_scene(sampled_objs, cfg):
...
def main(argv=None):
    ...
    if not camera_profile_json:
        raise ValueError("camera_profile_json must be provided in the config for bop_stereo_multidepth.")
    ...
    split_name = str(getattr(cfg, "dataset_type", "train_pbr"))
    if split_name == "train":
    ...
    dataset_dir = Path(cfg.output_dir).resolve() / cfg.bop_dataset_name
    split_dir = dataset_dir / split_name
    split_dir.mkdir(parents=True, exist_ok=True)
    lock = FileLock(str(split_dir / ".lock"))

    try:
        lighting_base_state = capture_render_lighting_state(light_point, light_plane_material)

        num_runs = int(getattr(cfg, "runs", 1))
        for run_idx in range(num_runs):
            random_cc_texture = np.random.choice(cc_textures)
            for plane in room_planes:
                plane.replace_materials(random_cc_texture)

            _prepare_objects_for_scene(sampled_objs, cfg)
            rig_poses_ir_left, _rig_poses_ir_right, rig_poses_color = build_rig_poses(cfg, rs)
            ...
            for pose_idx, (left_pose, color_pose) in enumerate(zip(rig_poses_ir_left, rig_poses_color)):
                ...
                with lock:
                    write_result = write_bop_multidepth(
                        cfg.output_dir,
                        target_objects=sampled_objs,
                        colors=[rgb],
                        ...
                        save_world2cam=True,
                        ignore_dist_thres=float(getattr(cfg, "ignore_dist_thres", 10.0)),
                ...
                print(
                    "[BOP Stereo MultiDepth] "
                    f"saved image_id={image_id:06d} | pose={pose_idx + 1}/{len(rig_poses_ir_left)}"
                )
