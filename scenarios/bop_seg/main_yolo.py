...
def main(args = None):
    ...
    while r < int(cfg.runs):
        ...
        bproc.object.sample_poses(objects_to_sample = sampled_objs,
        ...
        bop_bvh_tree = bproc.object.create_bvh_tree_multi_objects(sampled_objs)
        rotation_factor = 9.0
        poses = 0

        while poses < cfg.poses_cam:
            # Sample location
            radius_max = round(np.random.uniform(0.75, 2.5), 2)
        ...
        with lock:
            write_yolo_annotations(
                ...
                split=cfg.dataset_type,  # train/val/test
